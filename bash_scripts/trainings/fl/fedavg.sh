...
#SBATCH --output="logs/trainings/fl/fedavg/parent.out"

cd $HOME/repos/fl-varying-normalization

DIR_NAME=logs/trainings/fl/fedavg
echo $DIR_NAME
...
srun --ntasks=1 --cpus-per-task=1 --output="./$DIR_NAME/server.out" \
    $PLG_GROUPS_STORAGE/plggflmri/anaconda3/bin/python -m exe.trainings.run_server $PORT fedavg &
...
for norm_name in "${normalization_names[@]}"; do
    ...
    /net/pr2/projects/plgrid/plggflmri/Data/Internship/FL/varying-normalization/data/$norm_name $norm_name $PORT fedavg &
done
wait
